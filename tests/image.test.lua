...
test.it("names the format from the signature alone", function()
	test.equal(image.format(fixtures.png(WIDTH, HEIGHT, 4, fixtures.pattern)), "PNG")
	test.equal(image.format(fixtures.fixture("photo.jpg")), "JPEG")
	test.equal(image.format(fixtures.fixture("animation.gif")), "GIF")
	test.equal(image.format(fixtures.qoi(WIDTH, HEIGHT, 4, fixtures.pattern)), "QOI")
	test.equal(image.format(fixtures.bmp(WIDTH, HEIGHT, fixtures.pattern)), "BMP")
	test.equal(image.format("this is not an image"), nil, "text has no signature")
end)

test.it("knows what it can decode, even without a signature", function()
	local tga = fixtures.tga(WIDTH, HEIGHT, 4, fixtures.pattern)

	test.equal(image.format(tga), nil, "a targa has nothing to go by")
...
test.it("writes a png when nothing says otherwise", function()
	...
	test.equal(image.format(fixtures.read(path)), "PNG")
